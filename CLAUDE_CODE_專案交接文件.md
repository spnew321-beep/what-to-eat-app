# 這餐吃什麼 — Claude Code 專案交接文件 v4.3

> 這份文件取代先前版本。v2 新增了完整的「資料生命週期審查」(見第 8 節)。v3 一度把「成員標籤(people)」整個概念拿掉、併入「自訂標籤(tags)」——**這個決定在 v4 被推翻**,復原成「成員」跟「自訂標籤」兩套獨立系統。v4.2 拿掉了成員與餐廳之間唯一的連結(`restaurant_tags`)。**v4.3 再往前一步,把「群組」的假資料機制整個拿掉:**
>
> - **家人(`people`)**:結構化資料,不綁帳號,純粹是**你自己的飲食偏好備忘小抄**(名字/顏色/飲食偏好),用來記錄小孩、長輩這種不會有自己帳號的家人。**不貼餐廳、不能組群組、不能拿來發通知**——這是它現在唯一的用途。
> - **好友與群組**:原本用「成員標籤」模擬「一群人」的機制(`groups`/`group_members`)**整個拿掉**。原因是討論協作場次要「真的通知群組裡的人」時,才發現「拿假標籤模擬邀請對象」跟「這串資料要真的能收到通知」是兩件事,硬塞在一起只會製造之後帳號系統要接軌的麻煩。UI 上先留一個「🔒 加好友/建群組」的「尚未開放」提示,等 Tier 2+ 接上真帳號、真的能加好友互相確認關係之後,才會是「群組」這個詞該指的東西。
> - **自訂標籤(`custom_tags`)**:完全自由的文字,存在 `restaurants.custom_tags`(`text[]` 陣列)。「這間是誰喜歡的」這種備註統一用自訂標籤記錄(例如打「哥哥最愛」),系統不解析內容。
>
> 換句話說:v4 解決「成員該不該是結構化資料」(該);v4.2 解決「成員該不該連結到餐廳」(不該);v4.3 解決「沒有真帳號的『群組』該不該假裝存在」(不該——先誠實地顯示「尚未開放」,好過用假資料模擬出一個看似能用、實際上不能發通知的群組功能)。詳見第 2、5 節。這份文件本身已經是討論完的定案結果,不是待確認的草案,Claude Code 可以直接照著做,不需要再回頭跟使用者確認這些規則。
>
> 搭配同一批交付檔案裡的 `meal-picker.html`——那是目前唯一能跑的原型,UI 邏輯、互動細節都在裡面,已經套用 v4.3 的設計,建議直接讀那個檔案作為介面與互動邏輯的參考起點,這份文件負責補完原型沒有、也不可能有的部分(資料庫、帳號、排程、多人同步、真通知、真好友系統)。
...
- 用抽籤轉盤、系統建議、或多人協作場次,把「這餐吃什麼」這種每天都要吵的小決定變成好玩、公平、有效率的流程;協作場次目前靠邀請連結自由邀請,好友/群組通知是 Tier 2+ 才會開放的功能
...
### 關鍵原則(v4.3 定案):家人偏好備忘、真好友群組(未來)、自訂標籤,三套完全獨立、互不連結的系統

**家人(`people`)**——結構化資料,不是自由文字,但用途單純只剩一個:
- 有自己的欄位:`id`、`owner_user_id`(綁定帳號)、`name`、`color`、`prefs`(飲食偏好)
- **不貼餐廳**(沒有 `restaurant_tags`)、**不組群組**(沒有 `groups`/`group_members`)、**不能拿來發通知**
- 唯一用途:記錄不會有自己帳號的家人(通常是小孩、長輩)的飲食偏好備忘,例如系統建議模式未來可以參考「這個人不吃辣」。純粹是你自己帳號底下的參考資料,沒有身份狀態,不會被「代管」「認領」或「轉移」(v2 就定案,見第 8 節問題 2、8)

**好友與群組(Tier 2+ 才會實作,現在只有 UI 佔位)**:
- v2~v4.2 都還留著一套「拿 `people` 標籤模擬群組」的機制,讓使用者可以把假標籤湊成群組、協作場次選群組快速拉候選/設定通知對象。**v4.3 把這套假機制整個拿掉**——原因是這套資料格式（無帳號、無法通知、可任意捏造）跟「真的要通知一群人」的需求本質衝突,继续做下去只會在真帳號系統要接軌時製造更多要轉換的爛攤子
- 現在原型裡 👥 分頁跟協作場次的「選群組」入口都改成「🔒 尚未開放,需要帳號系統」的提示,誠實反映現況
- 未來的設計方向:真帳號可以互相加好友(雙向確認的真實關係,不是你單方面捏造的標籤),把好友分組之後,協作場次才能真的推播通知——這是全新的一套資料模型(例如 `friendships`、`decision_groups` 之類的表,參照真的 `auth.users`),**不是**把現有的 `people` 表加欄位改出來的,兩者的定位從一開始就不同
...
這三套系統**不要互相取代、合併或建立關聯**。演變過程:v3 把成員併入自訂標籤(拿掉結構化意義,已推翻);v4 復原成兩套獨立系統但成員還能貼餐廳;v4.2 拿掉成員貼餐廳的關聯;v4.3 拿掉假群組機制,家人只剩飲食偏好備忘這一個用途,群組讓位給尚未開放的真好友系統。

**協作場次裡真正會出現、會按✅、會表態的參與者,一律是透過邀請連結真的加入的人**(訪客或登入帳號),跟「家人」是兩個完全獨立的概念。目前唯一能用的邀請方式是「自由邀請」(分享連結);候選店家一律是手動瀏覽收藏挑,或用自訂標籤篩選快速加入。

---

## 3. 現有原型狀況(對照 meal-picker.html)

> **狀態更新(v4.3)**:`補充-UIUX優化定案.md`、`補充-食物類型擴充定案.md` 的內容都已經做進 `meal-picker.html` 原型裡了。**「群組」的假標籤機制已經拿掉,改成尚未開放的真好友系統佔位;成員(people)重新定位成純家人飲食偏好備忘**。下面的清單是**目前原型的實際狀態**。

已經做出來、邏輯完整、可互動的部分:
- 5 個分頁:🗺️ 地圖・⭐ 收藏・👥 群組・🎯 決策・☰ 更多(分頁名稱維持「群組」,保留給未來真好友/真群組系統;家人飲食偏好小抄移到 ☰ 更多分頁底下,是完全不同的功能,兩者不要混在同一個分頁)
- 餐廳收藏(CRUD 含編輯,不再是只能新增/刪除;地圖定位板;食物類型下拉篩選、自訂標籤下拉篩選,同一列並排)
- 每間餐廳可維護:自訂標籤(自由文字,可自己輸入任意詞,唯一的「誰喜歡」備註管道)、菜單存放區(常點餐點名稱+價格,可多筆增刪)、菜單照片(多張上傳,原型階段存在瀏覽器記憶體,重新整理會消失,正式版要接 Supabase Storage)
- 👥 群組分頁:整頁都是「🔒 加好友」「🔒 建立群組」尚未開放提示,誠實反映「這是保留給未來真帳號系統的位置」,沒有混雜任何家人資料
- ☰ 更多分頁新增「👨‍👩‍👧 家人偏好小抄」入口,跟自訂標籤管理/食物類型管理同一套列表管理樣式(新增/編輯/刪除家人飲食偏好),**不連結餐廳、不能組群組**,純粹是獨立的第三個「自訂項目管理」畫面
- 決策:個人抽籤、系統建議、協作場次(候選/表態合併在同一個子分頁、參與者另一個子分頁 → 轉盤結果)。抽籤與協作場次候選頁的自訂標籤 chip 是**篩選**用途(點了以後篩選下方瀏覽清單,使用者逐一勾選加入候選),不是整批匯入
- 協作場次的「這次要找誰一起決定」畫面只剩「🔗 自由邀請」可用,「🔒 選好友群組」是尚未開放提示
- ☰ 更多分頁有「自訂標籤管理」「食物類型管理」兩個入口(見第 4 節「資訊架構調整」說明),可以改名、刪除、看使用數量
- 決策結果可輸出成分享圖片(純 Canvas 繪製,是真功能)

明確是 placeholder、需要在 Claude Code 補上真功能的部分:
- 邀請連結、帳號系統、多裝置即時同步(目前是單機模擬)
- **真好友/真群組系統**(全新功能,不是把 `people` 改一改就好,見第 2 節說明)
- 隱私權政策/服務條款/帳號設定/意見回饋(各自有客製化的「尚未開放」提示文字,不是共用同一句)
- Google 地圖真實店家搜尋(API 金鑰不能放前端,暫不內建)
- 菜單照片的長期保存(要接 Supabase Storage)

**注意**:協作場次原本原型裡有「模擬新增訪客」「立即結算(測試用)」「把參與者存成新群組」幾顆 demo/假機制按鈕方便單機測試,都已經拿掉(正式版靠真實邀請連結 + 第 6 節的排程雙保險結算 + 未來的真好友系統)。這代表**這個原型現在沒有辦法在瀏覽器裡端到端測試「有人加入協作場次」的流程**,只能測試候選/表態/倒數這幾塊,要等 Tier 2 接上真帳號與邀請連結才能完整測試。
...
### 👥 群組(保留給未來真好友/真群組系統)
整頁只有「🔒 加好友」「🔒 建立群組」兩個尚未開放提示,外加說明文字。分頁名稱不改,因為這裡未來就是要放真的群組功能,不適合先借用來放別的東西。

### 👨‍👩‍👧 家人偏好小抄(在 ☰ 更多分頁底下,不是獨立分頁)
一份家人名單(名字/代表色/飲食偏好),新增/編輯/刪除,操作模式跟自訂標籤管理/食物類型管理統一(整列 + ✎/🗑,右上角「＋」新增)。**不分群組、不連結餐廳**,純粹是給小孩、長輩這種沒有自己帳號的家人留的飲食偏好備忘,跟上面的「群組」分頁是兩個完全獨立的東西,故意放在不同地方,避免使用者誤會兩者有關聯。
...
3. **協作場次**:見第 6 節詳細流程。「這次要找誰一起決定」畫面只剩「🔗 自由邀請」可用(分享連結),「🔒 選好友群組」是尚未開放提示;候選店家一律用自訂標籤 chip 快速加入,或手動瀏覽/新增,不會因為選了誰而自動帶入候選
...
## 5. 資料模型(已套用第 8 節審查結果 + v4.3:家人備忘/自訂標籤兩套獨立系統,群組讓位給未來真好友系統)

### 核心原則
- 餐廳收藏、地圖、決策歷史都是「帳號個人的」
- **家人(`people`)跟自訂標籤(`custom_tags`)是兩套獨立系統,不要合併、不要建立關聯**:家人是結構化資料(綁帳號、有 `prefs`),用途只剩「記錄沒有帳號的家人飲食偏好」;自訂標籤是完全自由的文字(只存在餐廳自己身上),用途是「這間店的備註,包括誰喜歡」
- 家人沒有身份狀態,不會被「代管」「認領」或「轉移」(v2 定案,見第 8 節問題 2、8),**不貼餐廳、不組群組、不能拿來發通知**——這三件事都被拿掉了,只剩飲食偏好備忘這一個用途
- **`groups`/`group_members`/`restaurant_tags` 這三張表都不用建**。真正的「好友/群組」是 Tier 2+ 才會設計的全新功能,參照真的 `auth.users`,不是拿 `people` 加欄位改出來的;現在的資料庫**不需要**先幫這個未來功能占欄位或建表,等真的要做的時候再設計
- 協作場次的參與者(真人)是完全獨立的世界,場次建立時參與者名單已是快照
- 每間餐廳除了基本資料,還有一份可多筆的菜單(常點餐點+價格)跟菜單照片

### 實體關聯(概念層級)
```
users(帳號)
  └─ 擁有 people(家人)、restaurants(收藏)、decision_history(歷史)、decision_sessions(場次)、custom_categories(自訂食物類型)

custom_categories(帳號自己新增的食物類型,跟前端寫死的 8 種預設類型並存)
  └─ 不對 restaurants.category 設外鍵,靠文字比對關聯,改名/刪除時應用層批次更新

people(帳號自己的家人飲食偏好備忘,結構化資料,綁帳號,沒有身份狀態、不會被認領或轉移)
  └─ **不連結 restaurants,不連結任何群組概念**——目前唯一的用途是飲食偏好參考資料
...
decision_sessions(協作場次)
  ...
  ├─ session_participants:真人(訪客或登入帳號),不是 people
  ├─ session_pool:候選店家,可以是某人收藏裡的店,也可以是場次專用臨時候選
  └─ session_votes:表態篩選模式專用

（未來 Tier 2+:friendships/decision_groups 之類的真好友系統,參照 auth.users,現在不用先設計）
```

### 正式的 Supabase (Postgres) SQL

```sql
-- 家人:結構化資料,綁帳號,沒有身份狀態,不會被認領或轉移,唯一用途是飲食偏好備忘
-- 不貼餐廳(沒有 restaurant_tags)、不組群組(沒有 groups/group_members)
create table people (
...
create table decision_sessions (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid references auth.users(id) on delete cascade not null,
  -- 沒有 group_id 欄位:目前沒有真群組可以參照,Tier 2+ 真好友系統做出來後,
  -- 要不要在這裡加邀請/通知對象欄位,等那時候的表結構定案再加,現在不用先占欄位
...
啟用 Row Level Security,`owner_user_id = auth.uid()` 是基本擁有者規則(`people` 也適用)。`restaurant_menu_items`/`restaurant_photos` 沒有自己的 `owner_user_id`,透過 `restaurant_id` 關聯回 `restaurants` 判斷擁有者。菜單照片建議直接用 Supabase Storage 存檔案本體,`storage_path` 只存路徑。`restaurants.category` 維持純文字欄位、不對 `custom_categories` 設外鍵(預設類型本來就不在資料庫裡,兩種類型都用文字比對),改名/刪除自訂類型時由應用層邏輯批次更新符合的 `restaurants.category` 文字值(改名→新名稱;刪除→「未分類」),做法比照 `decision_history.restaurant_name` 的文字備份模式。`decision_sessions`/`session_participants`/`session_pool`/`session_votes` 這幾張表因為訪客沒有帳號,需要另外設計「憑 `invite_token` 換取有限寫入權限」的規則(建議透過一個 Edge Function 驗證 token 沒過期,再用 service role 代為寫入,不要讓訪客直接拿到能繞過 RLS 的權限)。
...
```
1. Owner 進「決策」→「協作場次」
2. 選擇找誰一起決定:目前只有「自由邀請」(分享連結,空白開始)可以用;「選好友群組」是 Tier 2+ 接上真帳號、真好友系統之後才會開放的選項,現在原型裡是尚未開放的提示
   —— 不管選哪個,都**不影響候選店家怎麼湊**(候選一律用自訂標籤快速加入或手動瀏覽),也不影響誰會真的參與投票
...
11. 結果畫面有「🔒 加為好友」的尚未開放提示——這是 v4.3 拿掉「把參與者存成新群組」假機制之後留的位置,等真好友系統做出來,才會是「把這次一起吃飯的人加為好友」真正能用的功能
```
...
**v4→v4.3 修正紀錄**:上面第 6、9 題的定案結果(`decision_sessions.group_id` 用 `ON DELETE SET NULL`;結果畫面「存成新群組」按鈕)都是建立在「`groups` 表拿 `people` 標籤模擬」這個已經被拿掉的機制上,**目前都不需要實作**——沒有 `groups` 表,`decision_sessions` 也就沒有 `group_id` 欄位;「存成新群組」按鈕換成「🔒 加為好友」的尚未開放提示。第 2、5 節有完整說明,真正的群組要等 Tier 2+ 真好友系統做出來才會重新設計,屆時第 6、9 題的刪除規則需要照新的表結構重新定案,不能直接套用這裡的舊結論。
...
**為什麼選 Supabase**:關聯式資料庫對應「帳號→家人/餐廳→決策場次」的關聯結構;內建 Auth 解決帳號;內建 Realtime 解決協作場次的即時✅同步;Scheduled Functions/`pg_cron` 解決第 8 節的逾時排程需求,不用另外架服務。免費層額度(500MB DB、5 萬月活躍用戶、200 個並發 Realtime 連線)對這個規模綽綽有餘,唯一要注意免費專案連續 7 天沒存取會自動暫停(資料還在,手動喚醒即可)。
